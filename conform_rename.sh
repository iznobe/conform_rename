...
declare -i LongPath=0 NbRepScan=0 NbRepMod=0 NbFileScan=0 NbRepModified=0 NbFileModified=0 NbRepNOTModified=0 NbFileNOTModified=0; Debut=$(date +%s);
    ...
    if [[ "$nomOriginal" != "$nomModif" ]]; then # si il y a un changement a effectuer
        if test -d "$nomOriginal" ; then # si c' est un dossier
            ...
            if test -e "$nomModif" ; then # on verifie si il existe un dossier du meme nom avant de renommer
                NbRepNOTModified+=1
                echo "$NbRepScan un dossier du meme nom existe deja : $nomModif impossible de renommer $nomOriginal" >> /tmp/error.log
...
echo ""
echo "$NbRepScan dossiers et $NbFileScan fichiers traités, $NbRepModified répertoires modifiés, $NbFileModified fichiers modifiés"
...
echo "pour supprimer les dossiers vides , copiez collez la commande suivante : find '${execDir:=$PWD}' -type d -empty -delete"#!/bin/bash
